 ...
 *    Shows how to solve the diamond problem by using virtual inheritance so that the
 *    common base class appears only once in the derived hierarchy.
 *
 * 2. Covariant Return Types:
 *    Demonstrates how overridden methods in derived classes can have covariant return
 *    types, allowing the derived version to return a pointer/reference to a more specific type.
 *
 * 3. Avoiding Object Slicing:
 *    Highlights the slicing problem when copying derived objects into base objects and
 *    shows how to avoid it using pointers or references.
 *
 * ASCII Illustration:
 *
 *              Base
 ...
 *
 * Example:
 *   - Diamond Inheritance: Both Derived1 and Derived2 virtually inherit from Base,
 *     and FinalDerived inherits from both ensuring only one Base instance exists.
 *   - Covariant Return Type: FinalDerived::clone() returns a pointer to FinalDerived,
 *     overriding Base::clone() which returns a pointer to Base.
 *   - Object Slicing: Demonstrates that copying by value from a derived to a base object
 *     causes loss of derived-specific data; using pointers preserves full object.
 *
...
public:
    ...
    virtual Base* clone() const { return new Base(*this); } // For covariant return demonstration

protected:
    int baseValue;
};

// Derived1 and Derived2 virtually inherit from Base to solve the diamond problem.
...
void testObjectSlicing() {
    FinalDerived fd;
    // Demonstrate object slicing: storing a FinalDerived in a BaseHolder slices off derived parts.
