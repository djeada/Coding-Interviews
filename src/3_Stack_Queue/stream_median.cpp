 ...
 *
 * This program demonstrates three approaches for dynamically computing the median
 * of a stream of numbers as new elements are inserted:
 *
 * 1. Simple Solution:
 ...
 *    to maintain balance and enable efficient median retrieval.
 *    Complexity: O(log n) per insertion and O(1) median retrieval.
 *
 * 3. Alternative (Educational) Solution:
 *    Utilizes a balanced multiset with an iterator tracking the median. This approach
 *    allows logarithmic insertion and direct access to the median, showcasing another
 *    way to maintain order dynamically.
 *
 * ASCII Illustration:
 *
 ...
 *    Explanation: The median is defined as the middle number in a sorted list or the
 *                 average of the two middle numbers if the list has an even length.
 */
...
// Uses two heaps: a max-heap for the lower half and a min-heap for the upper half.
// Complexity: O(log n) per insertion, O(1) median retrieval.
