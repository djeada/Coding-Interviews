 ...
 *
 * Given an integer vector, count how many duplicates exist and list the
...
template <typename T> std::string vecToStr(const std::vector<T> &v) {
  std::string s = "{";
...
void runTests(const std::string &title, const std::vector<TestCase> &cases,
              Func func, Extract expectedExtractor, ToStr toString) {
  std::cout << "=== " << title << " ===\n";
  for (auto &tc : cases) {
    ...
    if constexpr (std::is_same_v<decltype(got), std::vector<int>>) {
      ...
      std::cout << tc.name << ": expected=" << toString(expected)
                << ", got=" << toString(got) << " -> "
                << (pass ? "PASS" : "FAIL") << "\n";
      assert(pass);
    } else {
...
int main() {
  ...
  runTests(
      "Testing countDuplicatesSimple", countCases, countDuplicatesSimple,
      [](auto &tc) { return tc.expectedCount; },
      [](auto v) { return std::to_string(v); });

  runTests(
      "Testing countDuplicatesSet", countCases, countDuplicatesSet,
      [](auto &tc) { return tc.expectedCount; },
      [](auto v) { return std::to_string(v); });

  runTests(
      "Testing countDuplicatesOptimal", countCases, countDuplicatesOptimal,
      [](auto &tc) { return tc.expectedCount; },
      [](auto v) { return std::to_string(v); });

  runTests(
      "Testing findDuplicatesOptimal", listCases, findDuplicatesOptimal,
