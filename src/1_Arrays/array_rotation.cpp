 ...
 *
 * The array was initially sorted in ascending order, then rotated at an unknown
 * pivot. The goal is to efficiently identify the minimum element.
 *
 * Visual Example:
 *
...
template <typename F>
void runTests(const std::string &title, F algo,
              const std::vector<TestCase> &cases) {
  std::cout << "=== " << title << " ===\n";
  for (const auto &tc : cases) {
    int result = algo(tc.arr);
    bool pass = (result == tc.expected);
    std::cout << tc.name << ": expected=" << tc.expected << ", got=" << result
              << " -> " << (pass ? "PASS" : "FAIL") << "\n";
    assert(pass);
  }
  std::cout << "\n";
