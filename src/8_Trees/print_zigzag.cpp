 ...
 * functionality to print the tree in a zigzag (spiral) level order. This means that the nodes
 * at each level are printed alternately from left-to-right and right-to-left.
 *
 * Approach:
 *   - Use two stacks to alternate between levels.
 *   - For the current level, push the child nodes into the alternate stack in an order that
 *     ensures the next level is printed in the opposite direction.
 *
 * ASCII Illustration:
 *           10
 *          /  \
 ...
 *
 * Example Output:
 * For the tree built by inserting {10, 5, 12, 11, 16}, the printed output might be:
 *   10 
 *   12 5 
 *   11 16
 *
 * Explanation:
 * The first level is printed left-to-right (just the root), the second level is printed
