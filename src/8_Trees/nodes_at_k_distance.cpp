...
int main() {
...

  {
    ...
    expectEqual(result, std::vector<int>{7, 15}, "distance 3");
  }

  {
    ...
    expectEqual(result, std::vector<int>{1}, "distance 4 left-skewed");
  }

  {
    ...
    for (const auto &value : {1, 2, 3, 4, 5})
      tree.add(value);
    const auto result = tree.findNodesAtDistance(4);
    expectEqual(result, std::vector<int>{5}, "distance 4 right-skewed");
  }

  {
    ...
    expectEqual(result, std::vector<int>{}, "distance beyond depth");
  }
