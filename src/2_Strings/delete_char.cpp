 ...
 *
 * Constraints:
 * - The input string may include letters, digits, spaces, or special
 * characters.
 * - The charsToDelete string may contain one or more characters.
 * - Aim for an efficient solution without relying solely on built-in replace
 * functions.
...
std::string simpleSolution(const std::string &input,
                           const std::string &charsToDelete) {
  std::string result;
  for (char c : input) {
...
std::string optimalSolution(const std::string &input,
                            const std::string &charsToDelete) {
  std::unordered_set<char> deleteSet(charsToDelete.begin(),
                                     charsToDelete.end());
  std::string result;
...
void runTests() {
  std::vector<TestCase> cases = {{"We are students", "aeiou", "W r stdnts"},
  ...
  for (const auto &tc : cases) {
    ...
    total++;
    if (checkResult(simpleSolution(tc.input, tc.charsToDelete), tc.expected,
                    "simpleSolution", tc))
      passed++;

    total++;
    if (checkResult(optimalSolution(tc.input, tc.charsToDelete), tc.expected,
                    "optimalSolution", tc))
      passed++;

    total++;
    if (checkResult(alternativeSolution(tc.input, tc.charsToDelete),
                    tc.expected, "alternativeSolution", tc))
      passed++;
    ...
    total++;
    if (checkResult(inplaceInput, tc.expected, "inplaceSolution", tc))
      passed++;
  }
