...
void inplaceSolution(std::string &s) {
  ...
  for (auto it = s.begin(); it != s.end(); ++it) {
    auto uc = static_cast<unsigned char>(*it);
    if (!seen.test(uc)) {
      seen.set(uc);
      *writer++ = *it;
    }
...
void runSuite(const std::string &solverName, SolverFn solver,
              const std::vector<TestCase> &cases, SuiteResult &outResult) {
  ...
  for (const auto &tc : cases) {
    ...
    std::cout << "  [" << (pass ? "PASS" : "FAIL") << "] " << std::left
              << std::setw(24) << tc.name << " expected=\"" << expected << "\""
              << ", got=\"" << got << "\"" << " (" << us << " µs)\n";
  }
  ...
  std::cout << "Summary for " << solverName << ": " << passed << "/" << total
            << " passed\n\n";
}
...
// Randomized cases
std::vector<TestCase> makeRandomCases(size_t count, size_t minLen,
                                      size_t maxLen,
...
int main() {
  ...
  std::vector<TestCase> coreCases = {
      ...
      {"Symbols", "@@##$$%%^^&&**(()))", "@#$%^&*()"},
      {"Long stress (100k)", makeLongStressInput(),
