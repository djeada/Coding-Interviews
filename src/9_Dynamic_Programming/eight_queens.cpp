 ...
 * N-QUEENS PUZZLE (N=8)
 *
 * Find all valid arrangements to place eight chess queens on an 8×8 chessboard such
 * that no two queens threaten each other. Queens threaten each other if they share
 ...
 * Input: N = 8
 * Output: 92
 * Explanation: There are exactly 92 distinct solutions for the 8-queens problem.
 */
...
int permutationSolution(int N) {
    ...
    do {
        if (isValid(columns, N)) solutionCount++;
...
int backtrackingSolution(int N) {
    ...
    std::function<void(int, std::vector<int>&, std::vector<bool>&, std::vector<bool>&, std::vector<bool>&)>
    solve = [&](int row, std::vector<int>& cols, std::vector<bool>& diag1, std::vector<bool>& diag2, std::vector<bool>& columns) {
        if (row == N) {
            solutionCount++;
            return;
        }
